...
function J = call_model_DRAM(pars,IC,tspace,NC,data,rflag);
if ~isstruct(pars)
    %     pars = exp(pars);
...
elseif rflag==2
    % RV P/V
    J = [sum(r_RVP.^2) sum(r_RVV.^2)];
elseif rflag==3
    % RV dynamic/ LV static
    J = [sum(r_RVP.^2) sum(r_RVV.^2) (rS_LVP_sys.^2+rS_LVP_dias.^2) (rS_LVV_sys.^2 + rS_LVV_dias.^2)];
else
    % RV/LV dynamic
    J = [sum(r_RVP.^2) sum(r_RVV.^2) sum(r_LVP.^2) sum(r_LVV.^2)];
end
